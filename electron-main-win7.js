...
ipcMain.handle('akid:get-user-guide', () => {
  try {
    return fs.readFileSync(path.join(__dirname, 'docs', 'USER_GUIDE_RU.md'), 'utf8');
  } catch (_) {
    return null;
  }
});
...
function createWindow() {
  const win = new BrowserWindow({
    ...
    title: 'Мониторинг работ АКИД/АКИП',
    webPreferences: {
      preload: path.join(__dirname, 'electron-preload-win7.js'),
